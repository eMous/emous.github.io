...
1. `-i`选项可以手动临时添加私钥，并且它将最先被尝试。之后将一次尝试 `ssh-add -L`中的私钥，最后尝试`.ssh/id_rsa`等。

2. 为什么只需要private key就行？

    - 在密码学的相关程序实现中，privatekey和publickey总是1-1对应的。
    - 在openssh的private key format中，包含了public key的信息。使用如下选项将private对应的public key输出到标准输出中。
...
一些希望站点在自己主机上登陆远程主机用户。这样不符合高的安全等级，但他确实很方便。这是一个可选的认证方式，在实现的时候一定要注意防止一个普通的用户有能力窃取到服务器的private host key。
...
**比较奇怪的是，协议中没有明确的规定如何返回这类认证被拒绝具体packet解释**,比如当远程host签名无效，又或者服务器不支持远程host的公钥算法等。
...
2. 非对称加解密的速度不及对称加解密，所以在SSH中的private/public key主要是用来认证的。
    ...
    2. public key可以从private key文件中导出。
