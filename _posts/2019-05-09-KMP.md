...
第三步：**讨论第二个元素的Next值**，从而决定第三个元素的Next值。发现第三个元素无法匹配时，第二个元素无法通过它自身的Next值对应的元素（索引为1的元素a）以及如此递归，构成相同首尾的串。因此第三个元素前方存不存在相同首尾串，因此其Next值为首元素索引。
...
<center><div style="text-align: center;"><img style="height:;width:100%;" alt="" title="" src="http://ss.showyoumycode.com/StaticData/Blog/KMP/next_step/0 04-49-07-8829.png"></div></center>
...
那为什么`T[Next[Next[i] + 1]]`不会等于`T[i+1]`呢？因为按照这样的算法**递推**可以确保从第一个元素开始计算，可以确保除了当前失配的元素意外，**之前的任意一个元素（索引为m）**都不可能等于`T[Next[m]+1]`。
